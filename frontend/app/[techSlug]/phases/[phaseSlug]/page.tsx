    ...
    <>
      ...
      <main id="main-content" className="container mx-auto px-4 py-10 max-w-4xl">
        ...
        <nav aria-label="Breadcrumb" className="flex items-center gap-1.5 text-sm text-muted-foreground mb-6">
          ...
          <Link href={`/${techSlug}/phases`} className="hover:text-foreground transition-colors">{tech.title}</Link>
          <ChevronRight className="h-3.5 w-3.5" aria-hidden="true" />
          ...
          <div className="flex items-center gap-6 px-6 py-8 sm:px-8 sm:py-10">
            <div className="flex-1 min-w-0">
              <div className="flex flex-wrap items-center gap-2 mb-3">
                ...
                <span className={cn('rounded-full px-2.5 py-0.5 text-xs font-medium', levelConfig.bgClass, levelConfig.textClass)}>
                  {levelConfig.label}
                </span>
              </div>
              <h1 className="text-2xl sm:text-3xl font-bold leading-tight mb-2 text-foreground">{phase.title}</h1>
              <p className="text-sm text-muted-foreground leading-relaxed max-w-lg">{phase.subtitle}</p>
            ...
            <div aria-hidden="true" className="hidden sm:flex shrink-0 h-20 w-20 items-center justify-center rounded-2xl bg-muted text-5xl">
              {phase.emoji}
            </div>
          </div>
        </div>
        ...
        {/* Progress (client) */}
        <PhaseDetailProgress techSlug={techSlug} phaseSlug={phaseSlug} totalLessons={phase.lessons.length} />
                  ...
                  {isPublished ? (
                    <div className="group flex items-center rounded-xl border border-border bg-card transition-all duration-200 hover:-translate-y-px hover:border-primary/25 hover:shadow-[0_4px_12px_oklch(0_0_0/0.05)] dark:hover:shadow-[0_4px_12px_oklch(0_0_0/0.3)]">
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="font-medium text-sm group-hover:text-primary transition-colors truncate">{lesson.title}</p>
