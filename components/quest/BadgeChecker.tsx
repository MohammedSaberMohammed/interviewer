...
export function BadgeChecker() {
  ...
  const challengeAnswers = useProgressStore((s) => s.challengeAnswers)

  // Streak-based badges — check whenever streak changes
  useEffect(() => {
    if (streakDays >= 7)  unlockBadge('streak-keeper')
    if (streakDays >= 30) unlockBadge('streak-legend')
  }, [streakDays, unlockBadge])

  // Study-time badges — checked on each lesson complete event
  useEffect(() => {
    function onLessonComplete(e: Event) {
      const { phaseSlug, lessonSlug, accuracy, completedLessons: all } = (
        e as CustomEvent<{
          phaseSlug: string
          lessonSlug: string
          accuracy: number
      ...
      if (phase13Done >= PHASE13_TOTAL) unlockBadge('trap-hunter')

      // First Trap Spotted — first Phase 13 challenge answered
      if (phaseSlug === '13-interview-traps') unlockBadge('first-trap-spotted')

      // Senior Certified — all 13 phases complete
      const TOTAL_LESSONS = 220
      if (all.length >= TOTAL_LESSONS) unlockBadge('senior-certified')
      ...
      if (hour >= 22 || hour < 4) {
        ...
        if (count >= 5) unlockBadge('night-owl')
      }
      if (hour >= 5 && hour < 8) {
        const key = 'early-bird-sessions'
        const count = parseInt(localStorage.getItem(key) ?? '0') + 1
        localStorage.setItem(key, String(count))
        if (count >= 5) unlockBadge('early-bird')
      }
      ...
      if (day === 6 || day === 0) {
        ...
        if (existing.has(0) && existing.has(6)) unlockBadge('weekend-warrior')
      }
    }
  ...
  // Accuracy-based badges — check when challenge answers change
  useEffect(() => {
    ...
    for (const stats of Object.values(byPhase)) {
      if (stats.total >= MIN_ANSWERS && stats.correct === stats.total) {
        unlockBadge('perfect-phase')
        break
      }
    }
  }, [challengeAnswers, unlockBadge])
