...
export function ChallengesExplorer({ challenges }: ChallengesExplorerProps) {
  ...
  // Group filtered challenges by phase → lesson
  const grouped = useMemo(() => {
    const byPhase = new Map<
      string,
      {
        ...
        lessons: Map<
          string,
          { lessonTitle: string; lessonHref: string; challenges: ExtractedChallenge[] }
        >
      }
    >()
        ...
        {selected ? (
          ...
          >
            <div className="mb-4 flex items-center gap-2 text-xs text-muted-foreground">
              <Link
                href={`/phases/${selected.phaseSlug}/${selected.lessonSlug}`}
